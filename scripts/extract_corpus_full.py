"""Full-corpus extraction driver.

Runs ``extract_corpus`` over every parallel chapter with maximum
parallelism. Logs progress to stdout (tee this to a file when invoking
from the shell). Writes ``data/extract_corpus_stats.json`` at the end
so we can inspect per-run stats without scraping logs.
"""
...
def main() -> int:
    ...
    print(f"=== extract_corpus_full ===", flush=True)
    ...
    stats = extract_corpus(
        parts=None,            # all parallel chapters
        rebuild=False,         # reuse the 4 cached chapters
    ...
    print(f"  cached:          {len(stats.parts_skipped_cached)}", flush=True)
